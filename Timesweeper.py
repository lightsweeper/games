    ...
    def heartbeat(game, sensorsChanged):
        ...
        if not game.board.is_playing and not game.animatingEnd:
            ...
            if game.endAnim.ended:
                ...
                if game.isWon:
                    score = int(time.time() - game.startTime)
                    game.display.clearAll()
                    timeScore = timedelta(seconds=score)
                    timeString = str(timeScore)
                    hours, minutes, seconds = timeString.split(":")
                    game.display.setMessage(2, "TIME", color = Colors.YELLOW, start = 1)
                    game.display.setMessage(3, "{:s}".format(seconds), color = Colors.YELLOW, start = 1)
                    time.sleep(5)
                    game.over(score)
                else:
                    game.over()
...
def main():
    gameEngine = LSGameEngine(Timesweeper)
