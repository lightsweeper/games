...
class Board():
    ...
    def count_surrounding(self, row_id, col_id):
        return sum(1 for (surr_row, surr_col) in self.get_neighbors(row_id, col_id)
    ...
    def has_visible_neighbor(self, row_id, col_id):
        for neighbor in self.get_neighbors(row_id, col_id):
            if neighbor[0] >= 0 and neighbor[1] >= 0 and neighbor[0] < len(self.board) and neighbor[1] < len(self.board[0]) and self.board[neighbor[0]][neighbor[1]].is_visible:
                return True
        return False
...
class EvilMinesweeper(LSGame):
    ...
    def heartbeat(game, sensorsChanged):
        ...
        # randomly place a new mine so that it will be noticed by the player
            if random.randint(0,120) == 0:
                rand_row = random.randint(0,game.rows-1)
                rand_col = random.randint(0,game.cols-1)
                i = 0
                iterating = True
                while iterating:
                    if game.board.board[rand_row][rand_col].is_visible == False:
                        if game.board.has_visible_neighbor(rand_row, rand_col) == True:
                            break
                    rand_row = random.randint(0,game.rows-1)
                    rand_col = random.randint(0,game.cols-1)
                    i += 1
